...
// @updateURL    https://raw.githubusercontent.com/werpswerps/jane-userscripts/refs/heads/main/jane-keyboard-shortcuts.user.js
// @downloadURL  https://raw.githubusercontent.com/werpswerps/jane-userscripts/refs/heads/main/jane-keyboard-shortcuts.user.js
...
window.addEventListener("keydown", function (e) {
  ...
  if (e.shiftKey && !e.metaKey && !e.ctrlKey && e.key === "<") {
    ...
    var settingsLink = document.querySelector(
      'a[data-tab-label="nav-btn-settings"]',
    );
  ...
  if (e.shiftKey && e.metaKey && !e.ctrlKey && e.key === "m") {
    ...
    if (!notice) return;
    var scribeBtn = Array.from(notice.querySelectorAll("button")).find(
      function (b) {
        return (
          b.textContent.trim() === "Pause" || b.textContent.trim() === "Resume"
        );
      },
    );
  ...
  // Cmd + Shift + K → Finish/Kill AI Scribe recording
  if (e.shiftKey && e.metaKey && !e.ctrlKey && e.key === "k") {
    e.preventDefault();
    var noticeFinish = document.getElementById(
      "patient-recordings-global-notice",
    );
    if (!noticeFinish) return;
    var finishBtn = Array.from(noticeFinish.querySelectorAll("button")).find(
      function (b) {
        return b.textContent.trim() === "Finish";
      },
    );
  ...
  if (e.shiftKey && !e.metaKey && !e.ctrlKey && e.key === "B") {
    e.preventDefault();
    var breakBtn =
      document.querySelector("#user-list-bottom button") ||
      document.querySelector('button[aria-label="Break"]');
...
var sparkles =
  '<svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="sparkles"' +
...
function buildKey(label) {
  return (
    '<span class="label label-light-grey"><strong>' + label + "</strong></span>"
  );
}
...
function buildItem(name, keys) {
  ...
  return (
    '<li tabindex="0">' +
    '<div class="row">' +
    '<div class="col-xs-6 text-right" style="display:inline-block;">' +
    name +
    "</div>" +
